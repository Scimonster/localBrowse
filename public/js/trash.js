function listTrash(files) {
    ...
    getDirContents('~/.local/share/Trash/info', {
        cont: true,
        simple: false
    }, function (trashinfo) {
        function action(f) {
            f = new LBFile(f);
            if (trashinfo({
                name: f.name + '.trashinfo'
            }).get().length) {
                var i = parseTrashInfo(trashinfo({
                    name: f.name + '.trashinfo'
                }).get()[0].cont),
                    last = $('<tr class="file">').appendTo('#file tbody');
                ...
                if (!s.expandHomeroot) {
                    last.find('td:last').html(function (x, old) {
                        return old.replace(homeroot, '~');
                    });
                }
                last.append('<td class="file-perm">' + f.permsFormatted() + '</td>');
                last.append('<td>' + f.permsFormatted() + '</td>');
                if (!f.readable) {
                    last.addClass('restricted');
                }
            }
        }
        if (s.dirFirst) {
