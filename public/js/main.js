...
function cd(loc, cb) {
	...
	function finishLoading() {
		...
		$('#filepath a:last').attr('href', function (i, old) {
...
function paste(files, destination, cut) {
	...
	paste.run(files, files.map(function (f) {
		return LBFile.addSlashIfNeeded(destination) + f.name;
	}), function (toPaste) {
		$.post('/mod', {
			action: cut ? 'move' : 'copy',
			files: toPaste
		}, function (pasted) {
				...
				$.get('info/info' + destination[i], function (dest) {
					...
					if (dest.exists) { // we'll need to overwrite/merge
						...
						function dataForFile(f, replace) {
							return '<div class="fileOverwriteDialog-fileInfo"><img src="' + imageForFile(f, true) + '"/><p>' + (f.type === 'directory' ? ('<b>' + _(replace ? 'paste-fileinfo-rfolder' : 'paste-fileinfo-efolder') + '</b><br/>' + _('paste-fileinfo-size', _('dirlist-filesize-items', f.items))) : ('<b>' + _(replace ? 'paste-fileinfo-rfile' : 'paste-fileinfo-efile') + '</b><br/>' + _('paste-fileinfo-size', f.filesizeFormatted()))) +
						...
						if (dest.type == 'directory' && src.type == 'directory') { // merge
							$.post('info/dir', 'simple=true&file=' + dest.dir, function (otherfiles) {
								...
								var d = jqUI.prompt({
									...
									text:
										'<div>' + _(src.date < dest.date ? 'paste-merge-body-newer' : 'paste-merge-body-older', (
									new LBFile(dest.dir)).name) +
										'</div>' + dataForFile(src, true) + dataForFile(dest, false) +
										'<p>' + _('paste-merge-body-newname') +
										'</p><p class="fileOverwriteDialog-exists">' + _('paste-merge-body-newname-exists') +
										'</p>',
							...
							$.post('info/dir', 'simple=true&file=' + dest.dir, function (otherfiles) {
								...
								var d = jqUI.prompt({
									...
									text:
										'<div>' + _(src.date < dest.date ? 'paste-overwrite-body-newer' : 'paste-overwrite-body-older', (new LBFile(dest.dir)).name) +
										'</div>' + dataForFile(src, true) + dataForFile(dest, false) +
										'<p>' + _('paste-overwrite-body-newname') +
										'</p><p class="fileOverwriteDialog-exists">' + _('paste-merge-body-newname-exists') +
										'</p>',
									buttonLabel: _('paste-buttons'),
									width: 500
...
function addBookmark() {
	...
	if (bookmarks.indexOf(file.path) + 1) {
		return;
	}
...
function sidebarTree(f) {
	...
	$.post('/info/treeParents', {
		...
		function scroll() {
			$('#sidebar-tree').scrollTop(curLocInTree.position() ? ~~curLocInTree.position().top : 0);
		}
		scroll();
		if (!$('#sidebar-tree').scrollTop()) {
			scroll();
		}
...
$(function () { // set up jqUI elements
	...
	$('#search-bttn').click(function () {
		if (file.size > 209715200) { // 200MB
			jqUI.confirm({
				text: _('search-large-body'),
				title: _('search-large-title'),
				width: 400
			}, function (c) {
				if (c) {
					run();
	...
	$('#new-dir, #new-file').click(function () {
		var newType = $(this).attr('id').substr(4),
			me = this;
		jqUI.prompt({
		...
		function (filename) {
			$.post(
				'/mod', {
				action: 'mk' + newType,
				file: file.addSlashIfNeeded() + filename
			},
	...
	$('a[href*="~"]').each(function () {
		$(this).attr('href', $(this).attr('href').replace('~', homeroot));
	});
	$('#locations').accordion({
		heightStyle: 'fill'
	});
...
$(d).ajaxError(function (e, jqxhr, settings, exception) {
	var url = settings.url.split('?')[0],
		split = obj.filter(url.split('/'), true);
	...
	var message = jqUI.alert({
		...
		})[url], (url == "info/localbrowseCWD" || url == "render/ctxMenu" || url == "/mod" ? '' :
			' ' + decodeURIComponent(getUrlVars('i?' + settings.data).file))) + (exception ? _('ajaxerror-exception', exception) : '') +
		...
		retryCountdown = setInterval(function () {
			...
			if ($(message.dialog).find('.retrying_in span').text() == 0) {
				message.buttons[0].click();
			}
		}, 1000);
});
