...
function listDir(files, beforeLoad, afterLoad) {
	...
	// set message
	$('#message').html(_('messages-dir-count', files({
		type: 'directory'
	}).count(), files({
		type: {
			'!is': 'directory'
		...
		' <span id=\"dirSize\">' + _('messages-dir-size', ($('#directorySize').text() || '...'), ($('#dirSizeDepth').text() || 3), location.hash) +
		'</span>');
	if (type != 'search') {
		// get dir size
	...
	$('#file .file').each(function () {
		$(this).data('info', new LBFile(files({
			path: $(this).data('path')
		}).get()[0]));
	});
	afterLoad();
...
$(d).on('click', '#fullDirSize', function () {
	jqUI.prompt({
		text: _('dirlist-depth-body'),
		title: _('dirlist-depth-title')
...
$(d).on('contextmenu', '#file .file', function (e) {
	...
	$('<ul id="contextMenu">').appendTo('body').offset({
		top: e.pageY,
		left: e.pageX
	}).
	html(jade.render('ctxMenu', {
		...
		prefix: 'selfile',
		ns: 'file',
		programs: programs.editorsForFile($('.file.sel').map(function () {
...
$(d).on('contextmenu', '#file.dirlist', function (e) {
	if ($(e.target).add($(e.target).parents('.file')).hasClass('file')) {
		return;
	}
	$('#contextMenu').remove();
	$('<ul id="contextMenu">').appendTo('body').offset({
	...
	html(jade.render('ctxMenu', {
		...
		null, {
			r: !(file.writable && sessionStorage.getItem('copy')),
			id: 'paste'
		}, {
			r: false,
			id: 'props'
	...
	appendTo('body').menu().offset({
		top: e.pageY,
		left: e.pageX
	});
});
$(d).on('click', '#contextMenu-folder-paste', function () {
	paste();
}); // no event object
$(d).on('click', '#contextMenu-file-props,#contextMenu-folder-props', function () {
	$.post('/info/info', {
		file: /folder/.test(this.id) ? file.path : $('.sel.last').data('path'),
		stat: true
	}, function (i) {
		i = new LBFile(i);
		var tabs = [{
			...
			locals: {
				...
				i: i,
				programs: programs,
				defProg: config.programs.defaults[i.type]
			},
			close: function () {
				$.post('/config', {
					newVal: $(':selected', 'select.openwith-program').val(),
					item: 'programs.defaults.' + i.type
				},
		...
		$('body').append(jade.render('properties/index.jade', {
			...
			file: i,
			compiled: tabs.map(function (tab) {
				return jade.render('properties/' + tab.file, tab.locals);
			}),
			_: _
		}));
		$('#props').tabs({
...
$(d).on('click', '#contextMenu-file-moveTo,#contextMenu-file-copyTo', function () {
	var sel = $('#file .file.sel').map(function () {
		return $(this).data('path');
	}).get(),
		id = this.id.substr(17, 4);
