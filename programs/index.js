...
exports.messages = arr.reduce(function (prev, cur) {
	return extend(true, prev, cur.messages);
}, {});
...
exports.allEditors = arr.map(function (ed) {
	return {
		modName: ed.modName,
		name: ed.name,
		desc: ed.desc
	...
	buttonFunction(file, function (buttons) {
		cb(buttons.map(function (b) {
			if (b instanceof Array) { // a buttonset
				return b.map(function (bsetB) {
					return buttonFromObject(bsetB);
				});
			}
